  ...
  if (path.startsWith('hit/ai/traces')) {
    ...
    if (mode === 'none') {
      return NextResponse.json(
        { error: 'Forbidden - access denied to AI traces' },
        { status: 403 }
      );
    } else if (mode === 'own') {
      // Traces don't have ownership fields, so 'own' mode denies access
      return NextResponse.json(
        { error: 'Forbidden - traces do not support ownership-based access' },
        { status: 403 }
      );
    } else if (mode === 'ldd') {
      // Traces don't have LDD fields, so 'ldd' mode denies access
      return NextResponse.json(
        { error: 'Forbidden - traces do not support LDD-based access' },
        { status: 403 }
      );
    } else if (mode === 'any') {
      ...
      return NextResponse.json(
        { error: 'Forbidden - invalid scope mode' },
        { status: 403 }
      );
    }
  }
