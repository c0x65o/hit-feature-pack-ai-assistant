...
export function AiOverlay(props) {
    ...
    const context = useMemo(() => ({
        ...
        // /dashboards/projects (dashboards are pack-scoped).
    ...
    const send = useCallback(async () => {
        ...
        try {
            ...
            try {
                ...
                const correlationId = agentData?.correlationId ? String(agentData.correlationId) : null;
                const serverError = agentData?.error || agentData?.detail || agentData?.message || null;
                const details = [];
                details.push(`Endpoint: ${endpoint}`);
                details.push(`Status: ${statusLine}`);
                if (correlationId)
                    details.push(`correlationId: ${correlationId}`);
                if (serverError)
    ...
    return (_jsxs("div", { style: containerStyle, children: [open && (_jsxs("div", { style: panelStyle, children: [_jsx("style", { children: overlayCss }), _jsxs("div", { style: {
                            ...
                            borderBottom: '1px solid var(--hit-border, rgba(255,255,255,0.12))',
                                        ...
                                        }, "aria-label": "Load trace", title: `Load trace ${lastCorrelationId}`, children: traceLoading ? 'Trace…' : 'Trace' })), _jsx("button", { onClick: () => {
                                            try {
                                                if (typeof window !== 'undefined')
