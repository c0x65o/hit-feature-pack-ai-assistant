...
function mismatchPenalty(q, m) {
    ...
    if ((qt.includes(' contact ') || qt.includes(' contacts ')) && !p.includes('/contacts'))
        pen += 15; // Increased penalty
    if ((qt.includes(' company ') || qt.includes(' companies ') || qt.includes(' customer ') || qt.includes(' customers ')) && !p.includes('/companies'))
        pen += 15; // Increased penalty
    if ((qt.includes(' deal ') || qt.includes(' deals ') || qt.includes(' opportunity ') || qt.includes(' opportunities ')) && !p.includes('/deals'))
        pen += 15; // Added deal penalty
...
export async function GET(request) {
    ...
    const filtered = packRaw && packRaw.toLowerCase() !== 'all'
        ? methods.filter((m) => String(m.featurePack || '').toLowerCase() === packRaw.toLowerCase())
        : methods;
